...
**Engineer:** Storage & Reliability  
**Status:** ✅ Phase 1 Complete - SQLite Primary Active

---

## Executive Summary

The PredictionStore abstraction is **LIVE and FUNCTIONAL** in production with SQLite as the primary backend. All prediction operations (BTC, ETH, XRP) are flowing through the abstraction layer successfully. PostgreSQL backend is fully implemented and ready for dual-write enablement.

**Current Production State:**
- ✅ SQLite primary backend: **ACTIVE**
- ✅ Dual-write mode: **DISABLED** (safe default)
- ✅ PostgreSQL backend: **IMPLEMENTED & READY**
- ✅ Abstraction layer: **100% OPERATIONAL**

**Production Evidence (from Railway logs Dec 1, 2025 @ 14:32 UTC):**
```
...
### 1.1 Backend Selection Logic (`core/prediction_store.py` lines 1-42)
...
**Default Behavior:**
- `PREDICTION_STORE_ENGINE` defaults to `"sqlite"` if not set
- `PREDICTION_DUAL_WRITE` defaults to `False` (0)
- SQLite path: `GHOST_PREDICT_DB` = `/app/data/ghost_predictions.db`
- PostgreSQL: `DATABASE_URL` = `postgres://...` (Railway Postgres)

**Backend Selection Algorithm:**
1. If `PREDICTION_STORE_ENGINE == "postgres"` AND `DATABASE_URL` is set → PostgresBackend
2. If `PREDICTION_STORE_ENGINE == "postgres"` BUT `DATABASE_URL` missing → **Fallback to SQLite** (with warning)
3. Otherwise → SQLiteBackend (default)

**Current Production Config:**
- `PREDICTION_STORE_ENGINE` = `"sqlite"` (or unset, defaults to sqlite)
- `PREDICTION_DUAL_WRITE` = `0` (disabled)
- Result: **SQLiteBackend primary, no dual-write**

---

### 1.2 Dual-Write Logic (`core/prediction_store.py` lines 63-120)

**Initialization (lines 54-62):**
```python
...
1. **SQLite Primary + Postgres Secondary:**
   - `PREDICTION_STORE_ENGINE=sqlite`
   - `PREDICTION_DUAL_WRITE=1`
   - `DATABASE_URL` is set
   - Writes: SQLite first, then Postgres (isolated errors)

2. **Postgres Primary + SQLite Secondary:**
   - `PREDICTION_STORE_ENGINE=postgres`
   - `PREDICTION_DUAL_WRITE=1`
   - Writes: Postgres first, then SQLite (isolated errors)

**Error Isolation (lines 100-109):**
```python
...
```
- Secondary write failures are **caught and logged**
- Primary backend success is **never blocked** by secondary failures
- Full stack traces logged for debugging

---

### 1.3 Service Layer Integration (`services/predictor.py`)
...
**Verification:**
✅ All prediction operations route through `_PREDICTION_STORE`  
...
### 2.1 Live Traffic Analysis (Railway Logs)

**Timestamp:** 2025-12-01 14:32:13 UTC

**Prediction Creation (ETH):**
```json
...
**Analysis:**
- ✅ Log message from `core.prediction_store` (line 267) confirms abstraction active
...
- ✅ 25 forecast points successfully stored

**API Endpoints Serving Live Data:**
```
...
**Interpretation:**
- Fast response times indicate SQLite reads are performant
...
**PostgresBackend Features (lines 417-831):**
...
- ✅ create_outcome() with INSERT ON CONFLICT (upsert)
...
- Indexes → Migrated (symbol+run_at, prediction_id+kind)
...
- **SQLite:** Per-request connections (10-15ms overhead)
...
### 3.2 Migration Tooling (`scripts/migrate_predictions_to_postgres.py`)
...
- ✅ Safe reads (no SQLite modifications)

**Migration Process:**
1. Count SQLite records (predictions, points, outcomes)
2. Verify PostgreSQL connection and schema
3. Migrate predictions in batches (default 100)
4. Migrate forecast points (executemany for speed)
5. Migrate actual points (with deduplication)
6. Migrate outcomes (if any exist)
...
### Phase 2A: Enable Dual-Write (SQLite Primary → Validation)

**Objective:** Validate PostgreSQL writes in production without switching primary backend.
...
**Expected Logs (Postgres Failure):**
```json
...
- [ ] Logs show `✅ Dual-write enabled: SQLite (primary) + PostgreSQL (secondary)`
...
- [ ] Secondary failures (if any) are logged but don't block primary

**Validation SQL (PostgreSQL):**
```sql
...
**Duration:** Run for **2-4 hours** during business hours to capture at least 4-6 prediction cycles (BTC/ETH cycle every 30-60 minutes based on logs).
...
**Objective:** Move existing SQLite predictions to PostgreSQL for unified history.

**Prerequisites:**
- Dual-write validation successful (2-4 hours clean logs)
- PostgreSQL confirmed receiving live writes
- No connection errors or timeouts

**Steps:**

1. **Dry-run migration (local or Railway shell):**
   ```bash
   cd /app
   python scripts/migrate_predictions_to_postgres.py --dry-run
   ```
...
2. **Set PREDICTION_STORE_ENGINE=postgres temporarily for migration:**
   ```bash
   export PREDICTION_STORE_ENGINE=postgres
   ```
   
   **⚠️ WARNING:** This tells migration script to target PostgreSQL. Do NOT deploy this to production yet - only for migration script execution.
...
3. **Execute migration:**
   ...
   **Expected Duration:** 1-3 minutes for 150 predictions
...
5. **Revert PREDICTION_STORE_ENGINE:**
   ```bash
   unset PREDICTION_STORE_ENGINE  # Back to sqlite for production
   ```

**Rollback (if migration fails):**
- SQLite data is **NEVER modified** by migration script
...
### Phase 3: Switch to PostgreSQL Primary

**Objective:** Make PostgreSQL the primary backend with SQLite as safety fallback.

**Prerequisites:**
- Dual-write validation clean (4+ hours)
- Historical migration successful (100% migrated, 0 failures)
- PostgreSQL query performance acceptable (<20ms reads)
- Team approval for cutover
...
2. **PostgreSQL Query:**
   ...
   **Expected:** Most recent predictions <60 minutes old

3. **SQLite Verification (dual-write secondary):**
   ```bash
   sqlite3 /app/data/ghost_predictions.db \
     "SELECT symbol, run_at FROM predictions ORDER BY run_at DESC LIMIT 3;"
   ```
   
   **Expected:** Same predictions as PostgreSQL (dual-write working)

**Duration:** Run for **24-48 hours** with dual-write before disabling SQLite fallback.
...
**Result:** Ghost immediately switches back to SQLite primary, zero data loss (all predictions in SQLite from dual-write).
...
**Objective:** Run PostgreSQL-only for maximum performance (remove dual-write overhead).
...
- Dual-write overhead removed (~10-15ms saved per prediction)
- Single connection pool (more efficient resource usage)
- Simplified logs (no dual-write messages)
...
**⚠️ Note:** After 48+ hours in PostgreSQL-only mode, SQLite data becomes stale. At that point, rolling back requires exporting PostgreSQL data back to SQLite (not covered here, but possible via migration script in reverse).
...
| Postgres-only (>48h) | Export Postgres → SQLite + switch | **Depends** (need export) | 5-10 minutes |

**Safety Guarantees:**
- SQLite file is **never deleted** by abstraction or migration
- Dual-write ensures **both databases always have data**
- Rollback is always a **config change + redeploy** (instant)
...
4. No errors in migration planning logic
...
**If Successful:**
- Confirms PostgreSQL backend can connect to Railway Postgres
- Confirms SQLite database is readable
- Validates migration script logic
- Provides exact counts for planning

**If Failed:**
- Reveals connection issues (DATABASE_URL misconfigured)
- Reveals missing dependencies (psycopg2)
- Reveals schema issues (table creation errors)

**This is a READ-ONLY operation** - no data will be written to either database.
...
| Data loss during rollback | None | N/A | Dual-write ensures both DBs always have data |

**Overall Risk Level:** **LOW** ✅

**Key Safety Mechanisms:**
- SQLite never modified by migration (read-only)
...
| `GHOST_PREDICT_DB` | `/app/data/...` | `/app/data/...` | `/app/data/...` | (unused) |

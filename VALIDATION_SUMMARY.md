...
- Adds `x-ghost-mw: on` header to all responses

**Global Exception Handlers** (line 205):
...
- All return structured JSON 500
...
**Code Ready**: No code changes needed, configuration only
...
- TTL: 30 minutes
- Disconnect detection built-in

**Timeout Issue**: Needs investigation after deployment
...
6. **Investigate SSE stream** (if still timing out):

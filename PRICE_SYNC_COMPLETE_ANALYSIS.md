...
- Price: **$24.37**
- NAV: **$205.19**
- Source: `"prev-close"` (yesterday's closing price)

**Difference**: 22¢ per share = **$1.85 NAV error**
...
**Problem**: Ghost caches prices for **45 seconds** during market hours.
...
**Problem**: If cache is "fresh" (< 45 seconds old), Ghost **never attempts to fetch
live prices**.
...
- Ghost will match your broker within 5-second window
...
```json
{
  ...
  "type": "shell",
  "command": "bash -lc 'source .venv/bin/activate 2>/dev/null || true; export PROMETHEUS_MULTIPROC_DIR=${PROMETHEUS_MULTIPROC_DIR:-/tmp/ghost_prom}; export PRICE_TTL_OPEN_S=5; mkdir -p \"$PROMETHEUS_MULTIPROC_DIR\"; uvicorn wolf_app:app --host 0.0.0.0 --port 5000 --reload'",
  "isBackground": true
}
```
...
Edit `wolf_app.py` line 512 directly:
...
- 100 shares @ $0.22 error = **$22 NAV discrepancy**
- 1000 shares @ $0.22 error = **$220 NAV discrepancy**
...
**Stale prices = Wrong trading decisions!**
...
2. Add `export PRICE_TTL_OPEN_S=5;` to task command
3. Restart task from VS Code

**Option 3: Code Fix (Permanent)**

1. Edit `wolf_app.py` line 512
2. Change default from `"45"` to `"5"`
3. Commit change to repo
...
Telegram | ✅ Fixed | Previous session |

**Final Action**: Restart Ghost with `PRICE_TTL_OPEN_S=5` to enable real-time price
synchronization.

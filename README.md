...
```bash
bash utils/wolf_quickstart.sh                 # uses HOST env (default http://127.0.0.1:5000)
...
Secrets reference: see `docs/ci_secrets.md`.
...
1. Check the [Issues](https://github.com/seancole713-source/GHOST/issues) page
...
1. Add a job to your Prometheus config:
   ...
   - job_name: ghost
      ...
      static_configs:
         - targets: ['ghost:5000']
   ```
2. (Optional) If you run multiple Gunicorn/Uvicorn workers, enable Prometheus
   multiprocess mode:
   ```bash
...
2. (Optional) Add an alert for persistent provider errors (Polygon / Alpha Vantage rate
   limits):
   ```yaml
...
histogram_quantile(0.5, sum(rate(ghost_cockpit_snapshot_build_seconds_bucket[5m])) by (le))
histogram_quantile(0.95, sum(rate(ghost_cockpit_snapshot_build_seconds_bucket[5m])) by (le))
sum by (provider)(rate(ghost_provider_fetch_seconds_sum[5m])) / sum by(provider)(rate(ghost_provider_fetch_seconds_count[5m]))
```

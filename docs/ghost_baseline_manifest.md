...
9. `GET /api/v3/predictions/latest?symbol=BTC&limit=3` - Latest predictions

**Cockpit V3 Endpoints** (Core UI Contract):
...
- `GET /api/v3/health/metrics` - System health metrics
...
9. Start daily reports (7 AM, 8 PM CT)
...
**Note**: `symbol` column does NOT exist (requires JOIN with `predictions` table)
...
- `core/ai_memory.py` - Contextual memory for agents

**Status**: Experimental modules not wired into auto-prediction loop
...
**CSS** (`static/css/cockpit_v3.css`):
- Dark theme
- Responsive grid layout
- Real-time animations

### 8.2 Expected Data Contracts
...
- Tier 4 (Low): 10s - VIP, alerts

**Benefit**: 60% load reduction (40-50 req/min)
...
3. Disabled by-symbol queries (symbol column doesn't exist in ghost_prediction_outcomes)
...
### 10.1 Alpaca Broker (`core/alpaca_broker.py`)

**Status**: ✅ Implemented, conditionally enabled
...
**Status**: Disabled in baseline (manual trading only)
...
**Enhancement Roadmap** (`COCKPIT_V3_ENHANCEMENT_ROADMAP.md`):
...
11. **Event Loop** (asyncio orchestration)
12. **AI Modules** (VIP scanner, XRP tracker, Stage 1/2/3)
13. **Watchlist Systems** (personal_watchlist.py)
14. **Risk Engines** (position_sizer.py, risk_engine.py)
15. **Orchestrator Logic** (service startup order)
...
10. ✅ **Always provide rollback paths** for any change

**Status**: 🛡️ **BASELINE GUARDIAN ACTIVE AND ENFORCED**

---

## 17. MANIFEST CHANGE LOG

...
Ghost Protocol v3 now uses **PostgreSQL**for prediction storage in production. Historical SQLite predictions have been
successfully migrated to the PostgreSQL backend on Railway.
...
Both SQLite and PostgreSQL use the same logical schema:**predictions**table:

- `id` (primary key)
- `symbol`, `run_at`, `horizon_h`
- `method`, `confidence`, `direction`
- `features_json`, `params_json`, `tag`**prediction_points**table:

- `id` (primary key)
- `prediction_id` (foreign key)
- `ts`, `kind` (forecast/actual), `price`**outcomes**table:
...
1.**Prediction Retrieval**(`/api/v3/predictions/latest`):

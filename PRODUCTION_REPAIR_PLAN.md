...
**Deployment Failed:** Healthcheck timeout caused by watchlist scheduler querying non-existent tables on startup.
...
**Why It Happened:**
1. ✅ Postgres connection working (pool initialized successfully)
2. ✅ Prediction store initialized with dual-write mode (Postgres + SQLite)
3. ❌ **Watchlist scheduler starts immediately** and tries to query `ghost_watchlist_items`
4. ❌ **Table doesn't exist** (fresh Postgres instance, no migration run)
5. ❌ **Errors cause app to hang** during startup healthcheck

---

## ✅ FIXES APPLIED

### Fix #1: Graceful Error Handling in Watchlist Scheduler

**File Modified:** `core/watchlist_prediction_scheduler.py`

**Changes:**
- Added graceful handling for missing database tables in 3 functions:
  ...
  - `_run_big_move_detection()` - Logs debug message instead of crashing

**Result:** Scheduler now **continues startup** even if watchlist tables don't exist yet.
...
**Expected:** Both return `{"ok": true, "prediction_id": ...}`
...
- `ghost_watchlist_items` - Main watchlist with symbols
...
**Result:** App starts successfully even if watchlist tables haven't been migrated yet.
...
1. Different error message (not watchlist-related)
2. Prediction store initialization errors
3. Postgres connection timeout/auth errors
...
**Solution:** Check POSTGRES_PASSWORD matches between services

**Issue:** `relation "ghost_predictions" does not exist`
**Solution:** Prediction store should auto-create tables, but if not:
```sql
...
**What We Fixed:**
- Made watchlist scheduler tolerate missing tables during startup
- App now starts successfully even with fresh/empty Postgres
- Migration can be run AFTER deployment is healthy

**What's Next:**
1. Push the fix → Railway deploys
2. Healthcheck passes → App is ACTIVE
3. Run migration → Watchlist features enabled
4. Set env vars → Scheduler activates

**Total Time:** ~5 minutes from push to fully operational
...
6. Cockpit UI: Loads without errors, shows predictions

**Watchlist features** are enabled when:

7. Migration run: All 4 watchlist tables created
8. `/api/v3/watchlist/user`: Returns 200 (not 404)
9. Can add/remove symbols via API
10. Cockpit shows: "Add Symbol" button in watchlist panel

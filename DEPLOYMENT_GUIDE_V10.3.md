...
2. **Logs**: Check `/tmp/ghost_test.log` for detailed errors
3. **Metrics**: Visit `/metrics` endpoint for system health
4. **Documentation**: See `CRYPTO_MODULE_QUICKSTART.md` for API examples
...
- ✨ Crypto prediction module (parallel to stocks)
...
- 🔧 Metrics registration moved to startup event
- 🔧 Cockpit API now includes `predictions.crypto` array
- 🔧 Database schema expanded for crypto predictions

**Fixed:**

- 🐛 Prometheus metrics not appearing in /metrics

...
### Patch 3: Verify /api/price/<symbol> uses ensure_price_cached

**Already correct** - Line 8948 shows:
...
### Patch 4: Confirm /api/portfolio and /api/position use cached snapshots

**Location check needed** - Verify these endpoints don't block on price providers.
...
2. **HIGH**: Enhance SSE stream with event types (status/ping/snapshot)
3. **MEDIUM**: Verify /api/portfolio and /api/position are non-blocking
4. **LOW**: Add /api/admin/cache/flush endpoint if missing

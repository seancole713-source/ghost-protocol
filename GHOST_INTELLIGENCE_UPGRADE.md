...
2. ❌ **Yahoo Finance rate limiting** (429 errors blocking real-time data)
3. ❌ **Stale cache** (returning yesterday's close instead of live price)
4. ❌ **No intraday volatility tracking** (missing high/low/volume data)
5. ❌ **Simple drift model** (doesn't account for gaps, momentum, volatility)

______________________________________________________________________

## 🎯 Improvement Strategy

### Phase 1: Fix Real-Time Data (IMMEDIATE)

**Priority**: 🔴 CRITICAL

1. **Switch primary provider from Yahoo to AlphaVantage**

   - Set `PRICE_YAHOO_FIRST=0` in Railway
   - AlphaVantage has 25 requests/day free tier (no rate limiting for hourly checks)

2. **Add intraday price tracking**

   - Fetch high/low/volume from AlphaVantage `TIME_SERIES_INTRADAY`
   - Store in `realized_prices` table with metadata

3. **Reduce cache TTL during market hours**

   - Current: 5 minutes
   - New: 1 minute during trading hours (9:30 AM - 4 PM ET)
   - Off-hours: Keep 5 minutes

**Expected Impact**: Real-time prices within 1 minute, no more stale $31.10
...
1. **Add volatility-aware forecasting**

   - Calculate ATR (Average True Range) from intraday data
   - Widen confidence bands when ATR > threshold
   - Example: 18% intraday range → 3x wider bands

2. **Detect and model price gaps**

   - Track overnight gaps (close → open)
   - Adjust drift based on gap direction
   - WOLF gap: $31.10 → $33.52 (+7.8%) then crashed

3. **Volume-weighted momentum**

   - High volume + price drop = strong sell signal
...
4. **News sentiment integration**
   ...
   - Weight: Recent news (< 1hr) gets 3x weight

**Expected Impact**: MAP drops from current ~15% to \<10%
...
3. **Feature importance analysis**
   ...
   - Drop weak features, amplify strong ones

**Expected Impact**: Ghost learns from mistakes, gets smarter over time
...
2. **Enhance forecast with volatility**:
...
3. **Real-time price override during market hours**:

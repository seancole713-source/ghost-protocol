...
if [ "$VARS_COUNT" -lt 5 ]; then
    ...
    echo ""
    echo "Run these commands (copy values from Railway → Variables or existing prod env):"
    echo ""
    echo "  railway variables set GHOST_API_TOKEN \"$(openssl rand -hex 32)\""
    echo "  railway variables set POLYGON_API_KEY \"$(railway variables get POLYGON_API_KEY --environment production)\""
    echo "  railway variables set ALPHAVANTAGE_API_KEY \"$(railway variables get ALPHAVANTAGE_API_KEY --environment production)\""
    echo "  railway variables set TELEGRAM_BOT_TOKEN \"$(railway variables get TELEGRAM_BOT_TOKEN --environment production)\""
    echo "  railway variables set TELEGRAM_CHAT_ID \"$(railway variables get TELEGRAM_CHAT_ID --environment production)\""

...
**Date:** 2025-10-14\
**Issue:** Telegram bot returning trading content for meta queries like "What time is
it"\
...
**This was the OLD code still running on Railway!** The fix we just made was only tested
...
Look for: **NO errors about missing environment variables**
...
3. **Telegram bot uses same code path** - It calls `_ask_ghost_ai()` just like the web
   API
4. **Now deployed** - Push to GitHub triggers Railway auto-deploy

______________________________________________________________________

## What Changed in Railway

**Environment Variables:** (Already correct, no changes needed)
...
**The fix is DEPLOYED to GitHub and Railway is auto-deploying NOW.**

Wait ~5 minutes for Railway deployment to complete, then test again in Telegram. You
should see:
...
**ETA:** 2-5 minutes for Railway auto-deploy\
**Verification:** Test "What time is it" in Telegram after deployment completes

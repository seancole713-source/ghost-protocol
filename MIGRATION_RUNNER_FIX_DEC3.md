...
### **Root Cause**The migration runner was crashing with `KeyError: 0` because it was trying to access PostgreSQL query results using**integer indices**(`result[0]`) instead of**column names**(`result['exists']`)

When `wolf_app.py` uses `psycopg2` with `RealDictCursor`, query results are**dictionaries**, not tuples. The code was
attempting tuple-style access on a dictionary, causing the KeyError.

### **Error Sequence in Production Logs**```text
...
###**Changes Made**1. ✅ Added `as exists` alias to SELECT EXISTS query (line 69)

1. ✅ Changed `result[0]` → `result['exists']` (line 72)
2. ✅ Applied same fix to `ensure_personal_watchlist_table()` function (line 147)
3. ✅ Improved exception handling to show error type for debugging (line 115)
...
###**Test 2: RealDictCursor Behavior**```python
...
```text**Expected:**No more `Database error: 0` or `KeyError: 0`
...
###**PostgreSQL Cursor Types**| Cursor Type | Result Format | Access Method | Used By |

|-------------|---------------|---------------|---------|
|**Standard**| `tuple` | `result[0]` | Most DB libraries |
|**RealDictCursor**| `dict` | `result['column_name']` | Ghost Protocol |

###**Why RealDictCursor?**Ghost Protocol uses `RealDictCursor` (configured in `core/db_engine.py` line 60) because
...
###**The Bug's Impact**- ❌ Migrations**never ran**in production

- ❌ Personal watchlist tables**never created**- ❌ Endpoint returned**404 Not Found**- ❌ Healthcheck**failed**(app marked unhealthy)
...
###**Why "KeyError: 0" and Not "KeyError: 'column_name'"?**When you access `dict[0]`, Python looks for the**key**`0` (integer), not index 0. Since RealDictCursor returns

`{'exists': True}`, key `0` doesn't exist.

###**Why Did This Pass Local Testing?**If local testing used
...
###**Alternative Fix Approaches Considered**1. ❌**Switch to standard cursor**→ Would break other code expecting dicts

1. ❌**Convert result to tuple**→ Extra overhead on every query
2. ✅**Use column names**→ Correct approach for RealDictCursor
...
###**Before Fix (Production Logs Dec 2)**```text
...
**Fix Applied:**December 3, 2025**Deployed By:**GitHub Copilot + Operator**Status:** ✅ Ready for Production Deployment

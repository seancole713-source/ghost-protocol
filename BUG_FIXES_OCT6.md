...
2. **Test Circuit Breaker Recovery**: Verify `/debug/reset_breakers` works when
   providers are stuck in backoff
3. **Validate Position Persistence**: Add/remove positions via API and confirm they
   survive server restart
4. **Price Provider Diversification**: Add IEX Cloud or Finnhub as fallback to reduce
   AlphaVantage/Yahoo dependency
5. **Portfolio State Reconciliation**: Add automated sync between ghost_state.json and
...
2. **Field Naming Consistency**: Position objects use different field names
   (`price_paid`, `entry`, `entry_price`) - need unified schema
3. **Type Safety Guards**: Even after checking `is not None`, need explicit variable
   extraction for type checker satisfaction
4. **Background Task Verification**: Add startup logging for all asyncio tasks to
   confirm they're scheduled
5. **Duplicate Code Detection**: Ctrl+F for function names after major changes to catch

...
**Logs show:** `KeyError: 'OPENAI_API_KEY'` or similar
**Fix:** Add all environment variables in Railway dashboard
...
**Logs show:** `ModuleNotFoundError: No module named 'xyz'`
**Fix:** Add missing package to `requirements.txt`

### Pattern 3: Port Binding
**Logs show:** `Address already in use` or port errors
**Fix:** Ensure CMD uses `${PORT}` variable

### Pattern 4: Database Path
**Logs show:** `sqlite3.OperationalError: unable to open database`
**Fix:** Add volume mount or use Railway database service

### Pattern 5: Redis Connection
**Logs show:** `redis.exceptions.ConnectionError`
**Fix:** Verify `REDIS_URL` is correct and Upstash Redis is active
...
2. **Check resource limits**: Your plan might be out of resources
3. **Try smaller deployment**: Disable non-critical features
4. **Check build time**: If > 10 minutes, may timeout

...
**Key Indicators:**
- ✅ `duration_ms` 1000-3000ms (normal for stocks - slower than crypto)
...
**Key Indicators:**
- ✅ Returns cached prediction from previous `/api/predict/run` call
...
Ghost uses a **modern, single-pipeline alert system** in `core/telegram_hunter.py` and `wolf_app.py`. No legacy systems exist.
...
**When this passes**: Telegram sends "⚡ SHORT-TERM GAINS" section with symbol, price, predicted gain, and confidence.
...
**Translation for Operator:**
- No predictions currently meet the quality thresholds (70%+ confidence, 2%+ gain)
...
**Schedule:**
- **Morning Report**: 7:00 AM (checks every 10 minutes)
- **Evening Report**: 8:00 PM (checks every 10 minutes)

**What Gets Included:**
1. Top opportunities meeting filter criteria (see above)
2. Accuracy statistics from `calculate_accuracy("24h")`
3. Market status (HOLDING PATTERN if no opportunities)
...
| **Daily Report** | 55% | None | MIN_ALERT_CONFIDENCE env var |
...
This means predictions with confidence ≥ 55% are included in daily reports, but only those ≥ 70% (short-term) or ≥ 75% (long-term) are sent as instant alerts.
...
2. **Outcomes created**: `outcomes` table (after 48h window closes)
3. **Accuracy calculated**: JOIN predictions + outcomes
...
**Translation:**
- Predictions exist but **48-hour window hasn't closed yet**
- Outcomes are created **only after 48 hours** via outcome reconciliation
- This is **NORMAL** for new deployments or after a restart
...
**Translation:**
- Predictions evaluated, but all directions were incorrect
...
**Day 1 (0-48 hours after deployment):**
```
...
**Confirmation:** ✅ Accuracy is **dynamically calculated** from database, never hard-coded.
...
#### STEP 1: Check Postgres Connection Logs
...
**Common Issues:**
- **Timeout errors**: Railway Postgres proxy is overloaded or down
- **Connection refused**: Postgres service not running
- **SSL errors**: Certificate issues (rare)

**Resolution:**
- Check Railway dashboard for Postgres service status
- Verify DATABASE_URL environment variable is set
- Check Railway incident status page

---

#### STEP 2: Restart Postgres (Railway Dashboard)

**When to use:** Persistent connection errors or timeouts

**Action:**
1. Log into Railway dashboard
2. Navigate to Ghost Protocol project
3. Find Postgres service
4. Click "Restart" button
5. Wait 30-60 seconds for service to stabilize
...
- ✅ Cache endpoints return latest predictions

**If tests pass:**
- System is healthy, previous issue was transient
...
#### STEP 4: Capture JSON + Log Snippet

**When to use:** Tests fail after restart, persistent errors

**Action 1: Capture Full Error JSON**
```bash
# Save error response
...
**Action 3: Check Prediction Store Status**
```bash
...
**Action 4: Escalate with Evidence**
...
**Share this with:**
- Development team via Slack/Discord
...
**Alternative Read Path (Cockpit V3):**
```
...
| `TELEGRAM_CHAT_ID` | None | Telegram channel/user ID |

...
**Before**: Accuracy tracking 0% functional (0 database records)  
...
3. Go to: Settings → Cron Jobs (or Deployments → Cron)
...
If Railway doesn't support cron, use external service:
...
**Expected for stocks** (PACS, etc.) - no stock API in standalone mode

**Unexpected for crypto** (BTC, ETH, SOL) - Coinbase API issue

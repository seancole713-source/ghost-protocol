...
Railway deployments were failing during health check phase with "service unavailable" errors, despite the application
starting successfully. All 6 health check attempts timed out within the 100-second window.
...
**Issue**: Missing `f` prefix on the SQL query string.
Without the f-string, Python treated `{ORDERS_TABLE}` as literal text instead of interpolating the variable value
`"orders"`.
...
{"message":"orders_init_error","error":"unrecognized token:
\"{\"","level":"warn","ts":"2025-11-22T18:48:27.362566+00:00"}

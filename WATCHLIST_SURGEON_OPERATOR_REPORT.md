...
**Mission:** Implement real Personal Watchlist alongside Market Watchlist in Cockpit V3  
**Status:** ✅ **COMPLETE - READY FOR PRODUCTION DEPLOYMENT**  
**Date:** December 2, 2025  
**Engineer:** Ghost Cockpit Watchlist Surgeon
...
2. **`wolf_app.py`** - **MODIFIED** (1 change)
...
3. **`api/personal_watchlist_endpoints.py`** - **ALREADY EXISTS** (verified working)
...
4. **`core/personal_watchlist.py`** - **ALREADY EXISTS** (verified working)
...
5. **`migrations/001_personal_watchlist.sql`** - **ALREADY EXISTS** (verified complete)
...
6. **`templates/cockpit_v3.html`** - **MODIFIED**
   - Added mode tabs: `#watchlist-mode-tabs` (Personal / Market)
   - Added filter tabs: `#watchlist-filter-tabs` (Stocks / Crypto / All)

7. **`static/cockpit_v3.js`** - **MODIFIED**
   - Added `watchlistMode` and `watchlistFilter` state variables
...
8. **`static/personal_watchlist_ui.js`** - **MODIFIED**
   - `loadPersonalWatchlist()` - Fetches `/api/v3/watchlist/user`
...
9. **`test_watchlist_endpoints.py`** - **CREATED**
...
**Auto-Migration Runner:** ✅ IMPLEMENTED
...
**How it works:**
1. On app startup, checks if `ghost_watchlist_items` table exists
2. If missing, executes `migrations/001_personal_watchlist.sql`
3. If already exists, logs "already applied" and continues
4. Logs each step: `[MIGRATION] ✅ 001_personal_watchlist.sql - applied successfully`
...
**2. `watchlist_prediction_tracking`** (Prediction history)
- Tracks every prediction generated for watchlist symbols
- Links to `ghost_predictions` table via `prediction_id`
- Used for history view in UI

**3. `watchlist_price_snapshots`** (Price tracking)
- High-frequency price snapshots (every 15 minutes)
- Used for big-move detection

**4. `watchlist_alerts_log`** (Alert history)
...
### 1. GET `/user` - Fetch Personal Watchlist
...
**Enrichment Details:**
- `current_price`: Live price from turbo providers (Polygon/CoinGecko)
...
### 2. POST `/add` - Add Symbol to Watchlist
...
**If re-activating soft-deleted symbol:**
```json
{
  "ok": true,
  "action": "re-activated",
...
### 3. POST `/remove` - Remove Symbol from Watchlist
...
**Note:** This is a **soft delete** (sets `active = FALSE`). Symbol can be re-added later and will reuse the same ID.

### 4. POST `/update-position` - Toggle Ownership Flag
...
### 5. GET `/history/{symbol}` - Get Prediction History
...
### Panel 5: Watchlist (Two-Tab System)

**Default view on load:**
- Mode: "📋 Personal" (active by default)
- Filter: "All" (shows both stocks and crypto)

### Tab 1: "📋 Personal" - My Watchlist
...
**Controls:**
- **➕ Add Symbol**: Opens modal with form (symbol, asset type, ownership, notes, alert threshold)
...
### Tab 2: "📊 Market" - Market Watch

**Read-only, shows default Ghost symbols:**
```
┌──────────────────────────────────────────┐
...
**Data source:** `/api/v3/watchlist/enriched` (existing endpoint)

### Filter Tabs (Work in Both Modes)

**Stocks:** Shows only `asset_type = 'stock'` or `type = 'stock'`  
**Crypto:** Shows only `asset_type = 'crypto'` or `type = 'crypto'`  
...
**Expected output:** `0` (table exists, no items yet)

**Alternative (via Railway Postgres plugin):**
...
### Step 3: Test Cockpit UI

**Open:** `https://ghost-protocol-production.up.railway.app/cockpit/v3`

**Test checklist:**
- [ ] Panel 5 shows two mode tabs: "📋 Personal" and "📊 Market"
...
- ⏳ **NEEDS DEPLOY** to trigger migration

**Frontend:**
- ✅ Dual-mode tabs implemented
- ✅ Add/remove controls wired
- ✅ Filter tabs working in both modes
- ✅ Market watchlist preserved
- ⏳ **NEEDS DEPLOY** to push to production
...
### 2. Add Your Personal Symbols

**Add your special watchlist (DOGE, PEPE, FLOKI, SHIB, XRP):**
...
**Expected:**
- Panel 5 → Personal tab shows 5 symbols
...
- Catches "already exists" errors and logs success
...
**Error Handling:**
- Migration failures are logged but **non-blocking**
- App continues startup even if migrations fail
- This prevents cascade failures in production

### Price Enrichment Strategy

**Priority order:**
1. Latest prediction from `ghost_predictions` table (includes current_price)
2. Live price from turbo providers (Polygon for stocks, CoinGecko for crypto)
3. NULL if both fail

**Caching:**
- Predictions cached in prediction_store (15-30 min TTL)
- Turbo providers have built-in rate limiting (3-5s timeout)

### Performance Considerations

**Database queries:**
- `get_watchlist()`: Single SELECT with ORDER BY priority, indexed
- `get_enriched_watchlist()`: Loops through items (N queries)
- **Optimization opportunity:** Batch prediction fetch in single query

**UI refresh rate:**
- Personal watchlist: Every 15 seconds
- Market watchlist: Every 15 seconds
- Prediction data: Every 15 seconds (via separate endpoint)

**Recommended max items:**
- Personal watchlist: 50 symbols (UI shows first 20)
...
- All watchlist items belong to "the operator" (single user)
- No user_id or authentication
- Perfect for personal Ghost instance
- If multi-user needed: Add `owner_key` column + auth middleware

### VIP Coins Not Auto-Added

**Per mission constraints:**
- VIP coins (WEPE, LILPEPE, DORKL, SLOTH, APC) are **NOT** auto-added to personal watchlist
...
### Issue: GET /user returns timeout or 500 error

**Cause:** Migration hasn't run yet (table doesn't exist)
...
### Issue: UI shows "Your watchlist is empty" after adding symbols
...
### Issue: Prices show `--` (null)

**Cause:** Price provider failed (rate limit, invalid symbol, or API timeout)

**Fix:**
- Normal for invalid symbols (e.g., "XXX" is not a real ticker)
- Check symbol spelling (AAPL not APPLE, BTC not BITCOIN)
- For valid symbols: Provider may be rate-limited (wait 60s and refresh)

### Issue: "Add Symbol" button does nothing

**Cause:** JavaScript error blocking modal
...
### Issue: Market tab empty

**Cause:** `/api/v3/watchlist/enriched` endpoint failing

**Fix:**
- This is existing endpoint, should always work
...
10. ✅ Zero hard-coded personal symbols
11. ✅ No breaking changes to existing features
12. ✅ Matches existing Cockpit design (fonts, colors, badges)
...
```bash
git add core/migration_runner.py wolf_app.py templates/cockpit_v3.html static/cockpit_v3.js static/personal_watchlist_ui.js test_watchlist_endpoints.py WATCHLIST_SURGEON_OPERATOR_REPORT.md
git commit -m "Add personal watchlist system with auto-migrations"
git push origin main
```

**2. Verify Migration Applied** ⏳ **REQUIRED**
```bash
...
- Market watchlist behavior unchanged (read-only, default Ghost symbols)
...
- SIM_MODE=0 preserved (100% live data)

**How to Use:**
1. Open Cockpit V3 → Panel 5
2. Click "📋 Personal" tab
3. Click "➕ Add Symbol"
4. Add symbols (e.g., BTC, AAPL, DOGE)
5. View live prices + Ghost predictions
6. Remove symbols with "✖" button
7. Switch to "📊 Market" tab to see default Ghost symbols
...
**Contact:** Ghost Cockpit Watchlist Surgeon  
**Date:** December 2, 2025
...
### 1. `templates/cockpit_v3.html` (Watchlist Panel Structure)

**Changes:**
- Added **Mode Tabs** (`#watchlist-mode-tabs`): Personal vs Market
- Kept existing **Filter Tabs** (`#watchlist-filter-tabs`): Stocks/Crypto/All
- Both tab groups visible in Panel 5
...
**Impact:** Users can now switch between Personal and Market watchlists using dedicated tabs.

---

### 2. `static/cockpit_v3.js` (Core UI Logic - Refactored)
...
#### `loadWatchlistByMode()` - Master Router
```javascript
...
#### `switchTab()` - Handles Both Mode Tabs and Filter Tabs
```javascript
...
**Impact:** Market watchlist behavior preserved exactly as-is, personal watchlist delegated to separate module.

---

### 3. `static/personal_watchlist_ui.js` (Personal Watchlist Module - Updated)

**Removed Auto-Override:**
```javascript
// OLD (removed):
...
- `toggleOwnership()` - Update owns_position flag
- `viewSymbolHistory()` - View prediction history modal

**Impact:** Personal watchlist module now works **alongside** cockpit_v3.js instead of overriding it.
...
| **POST** | `/api/v3/watchlist/add` | Add symbol to watchlist | `{symbol, asset_type, owns_position, notes, alert_threshold_pct, priority}` |
...
| **GET** | `/api/v3/watchlist/history/{symbol}` | Get prediction history | Query param: `limit` (default 50) |

**Response Example** (`/api/v3/watchlist/user`):
```json
{
  "items": [
    {
...
| **GET** | `/api/v3/predictions/latest?limit=100` | Fetch predictions for enrichment |

**Response Example** (`/api/v3/watchlist/enriched`):
```json
{
  "ok": true,
  "items": [
...
### Default Load (Mode: Personal, Filter: All)

**When user opens `/cockpit/v3`:**
1. Cockpit loads with "📋 Personal" tab active
2. `loadWatchlistByMode()` detects `watchlistMode = 'personal'`
3. Calls `loadPersonalWatchlist()` from `personal_watchlist_ui.js`
4. Fetches `/api/v3/watchlist/user`

**If personal watchlist is empty:**
```
┌──────────────────────────────────────────┐
│  📋 Your watchlist is empty              │
│                                           │
...
**If personal watchlist has items:**
```
┌──────────────────────────────────────────┐
│  [➕ Add Symbol]           (top-right)   │
│                                           │
...
**On submit:**
1. POST to `/api/v3/watchlist/add`
2. Modal closes
3. Watchlist reloads
4. Toast notification: "✅ {SYMBOL} added to watchlist"
...
**Data source:** `/api/v3/watchlist/enriched` (default Ghost symbols)
...
**Personal Mode + "Stocks" filter:**
- Shows only symbols where `asset_type = 'stock'`
- Filtered by `personal_watchlist_ui.js` via `getFilteredWatchlistItems()`

**Personal Mode + "Crypto" filter:**
- Shows only symbols where `asset_type = 'crypto'`

**Market Mode + "Stocks" filter:**
- Shows only symbols where `type = 'stock'`
- Filtered by `loadMarketWatchlist()` using `Array.filter()`

**Market Mode + "Crypto" filter:**
- Shows only symbols where `type = 'crypto'`

---

## DATABASE SCHEMA

**Table:** `ghost_watchlist_items` (Postgres)
...
**Migration:** `migrations/001_personal_watchlist.sql` (153 lines)
...
### Market Watchlist Backward Compatibility

**Old behavior preserved:**
- Shows default Ghost symbols (BTC, ETH, DOGE, ADA, XRP, SOL, etc.)
- Enriched with 48h predictions from `/api/v3/predictions/latest`
- Displays price change % and Ghost confidence %
- Updates every 15 seconds
- Filter tabs (Stocks/Crypto/All) work

**What changed:**
- Now accessible via "📊 Market" tab instead of being the default
...
**Step 1: Check Endpoints Respond**
```bash
...
**Step 2: UI Loads Without Console Errors**
1. Open: https://ghost-protocol-production.up.railway.app/cockpit/v3
2. Open Browser DevTools (F12) → Console tab
3. Expected logs:
   - `[PERSONAL WATCHLIST] UI module initialized and ready`
   - NO red errors about "undefined function" or "cannot read property"

**Step 3: Test Add Symbol**
1. Click "📋 Personal" tab (if not already active)
2. Click "➕ Add Symbol"
3. Fill form:
   - Symbol: `AAPL`
   - Asset Type: `Stock`
   - Alert Threshold: `5.0`
4. Click "Add Symbol"
5. Expected: Modal closes, AAPL appears in watchlist, toast notification shows

**Step 4: Test Remove Symbol**
1. Click "✖" button next to AAPL
2. Confirm removal
3. Expected: AAPL removed, toast notification shows

**Step 5: Test Market Watchlist**
1. Click "📊 Market" tab
2. Expected: Default Ghost symbols appear (BTC, ETH, DOGE, etc.)
3. NO add/remove buttons visible

**Step 6: Test Filters**
1. In Personal mode: Click "Stocks" → only stocks shown
2. Click "Crypto" → only crypto shown
3. Switch to Market mode: Repeat → filters still work

**Step 7: Verify Persistence**
1. Add 3 symbols to personal watchlist
2. Refresh browser (F5)
3. Expected: All 3 symbols still visible (not reset to empty)
...
**Impact of rollback:**
- Watchlist reverts to "market only" behavior (no personal watchlist)
...
10. ✅ Filter tabs (Stocks/Crypto/All) work in both modes
...
1. **Add symbols to your personal watchlist:**
   - Open Cockpit V3 → Panel 5 (Watchlist)
   - Click "📋 Personal" tab (default)
   - Click "➕ Add Symbol"
   - Add: DOGE, PEPE, FLOKI, SHIB, XRP (your special watchlist from requirements)

2. **Toggle between Personal and Market:**
   - "📋 Personal" → Your custom symbols with add/remove controls
   - "📊 Market" → Default Ghost market movers (read-only)

3. **Use filters:**
   - "Stocks" → Show only stock symbols
   - "Crypto" → Show only crypto symbols
   - "All" → Show everything

4. **Manage ownership:**
   - Click "✅" button to mark symbol as owned
   - Click "➕" button to mark symbol as not owned
   - Ownership badge appears next to symbol

5. **View prediction history:**
   - Click "📊" button on any watchlist row
   - Modal shows historical predictions for that symbol

**What persists:**
- All symbols you add to personal watchlist
- Ownership flags
- Notes you write
- Alert thresholds
- Survives: browser refresh, app restart, Railway redeployment

**What does NOT persist:**
- Filter tab selection (resets to "All" on refresh)
...
- Save filter/mode selection in `localStorage` (5 lines of JS)
...
**Next Step:** Deploy to Railway → Test in production → Report results  
**Estimated Deployment Time:** 2-3 minutes (Railway auto-build + restart)

...
The env vars `TELEGRAM_BOT_TOKEN` and `TELEGRAM_CHAT_ID` are loaded **once at startup**
(line 806-807 in wolf_app.py):
...
3. **Redeploy** the service (Railway → Deployments → Redeploy)
4. Wait 2-3 minutes for deployment
5. Test: `curl -X POST https://web-production-8e9a0.up.railway.app/api/telegram/test`
...
instance never saw them. Environment variables need to be present **before** or
**during** deployment to be loaded at startup.

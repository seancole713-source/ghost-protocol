...
**Status:** 4/12 CRITICAL patches applied ✅  
**Commit:** 4ff519b  
...
**Issue:** XRP tracker returns `confidence: 0.0` always (not connected to predictions)
...
### Patch 6: Implement LIVE/FIXED Mode Toggle
**Priority:** 🔴 HIGH  
**Files:** `wolf_app.py` + `static/cockpit_v3.js`  
**Issue:** Mode selector is cosmetic only (no backend effect)
...
**Test:**
1. Toggle LIVE → FIXED in UI
2. Verify console shows POST request
3. Verify panels reload

---

### Patch 7: Debug Top Movers Empty Display
**Priority:** 🔴 HIGH  
**Files:** `static/cockpit_v3.css` (investigate), `static/cockpit_v3.js` (add logging)

**Issue:** Data loads correctly (`/api/v3/hunter/feed` returns movers) but DOM shows empty container
...
**Likely Fix:**
- Remove `display: none` from `.mover-card`
- Increase `.panel-body` min-height
- Ensure `.movers-grid` has proper grid layout

---

### Patch 8: Fix News Feed Sentiment Display
**Priority:** 🟡 MEDIUM  
**File:** `static/cockpit_v3.js`

**Issue:** All news items show "Neutral" even though API returns "bullish"/"bearish"
...
### Patch 9: Show Ghost Signals on Major Caps
...
**Enhancement:** Dedicated presale awareness surface

**Template Addition (cockpit_v3.html):**
```html
...
### Patch 11: Prediction Accuracy Placeholder
**Enhancement:** Show message when no data available
...
### Patch 12: Watchlist Action Buttons
**Enhancement:** Wire up ➕ (mark owned), 📊 (history), ✖ (remove)

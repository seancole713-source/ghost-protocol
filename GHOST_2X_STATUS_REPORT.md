...
- ❌ Missing 5/8 stocks: **NVDA, GOOGL, AMZN, TSLA, META**
...
### 1. **Missing Stock Predictions** 🚨 HIGH PRIORITY
**Issue**: Only 3/8 stocks generating predictions (37.5% success rate)
...
### 2. **Crypto Predictions Disabled** 🚨 HIGH PRIORITY
**Issue**: 0/8 crypto predictions generating
...
### 3. **Ghost Score Not Public** ❌ MEDIUM PRIORITY
**Issue**: Ghost Score V2 calculated but not exposed in public endpoints
...
### 4. **Missing Cockpit Modules** ❌ LOW PRIORITY
**Issue**: Baseline Ghost Protocol modules not yet implemented
...
- ❌ Presale Sniper Prep Panel

**Status**: Not implemented yet, not blocking core functionality

---

### 5. **Protected Endpoints** ⚠️ MEDIUM PRIORITY
**Issue**: Some endpoints require Bearer token but should be public
...
2. Test multi-run endpoint:
   ...
   ```
3. Expected result: `{"stocks": 6-8, "crypto": 0, "vip": 4}`
4. If still only 3 stocks, check Railway logs:
   ```bash
...
1. Test individual predictions:
   ...
   ```
2. Verify non-WOLF symbols work correctly
3. Confirm no "DELISTED MODE" errors

...
4. Select **"Redeploy"**
5. **✅ IMPORTANT**: Check the box **"Clear build cache"**
...
- Path: `/health`
- Timeout: `300` seconds (or 15 seconds as suggested)

**Environment Variables:**

- PORT should be provided automatically by Railway
...
**Note**: Railway provides PORT via environment variable, so it might be 8080 or another
port (NOT 8444).
...
curl -s $BASE/openapi.json | python3 -c "import sys,json; d=json.load(sys.stdin); print(f'Total routes: {len(d[\"paths\"])}'); news=[p for p in d['paths'] if p.startswith('/api/news')]; print(f'News routes: {len(news)}'); [print(f'  {p}') for p in news]"
...
- Import error: Verify `routes/news_routes.py` exists in repo

**Solutions:**

- If missing file: Ensure `routes/news_routes.py` is in git
...
### If Deploy Fails (healthcheck timeout)

**Problem**: Application starts but healthcheck fails

**Solutions:**

1. **Temporarily disable healthcheck**:

   - Settings → Deploy → Healthcheck Path → **Leave empty**
   - Redeploy
...
**Problem**: Commits `b5b3a3e`, `a31ce8b`, etc. don't appear in Deployments list
...
5. Select repository: `seancole713-source/GHOST`
6. Select branch: `main`
7. Enable **"Deploy on push"**
8. Manually trigger **"New Deployment"** → **"From GitHub"** → `main` branch
...
1. Railway dashboard shows commit `b5b3a3e` as **"Active"**
2. Build logs show successful pip install
3. Deploy logs show "Application startup complete"
4. HTTP logs show "GET /health HTTP/1.1 200 OK"
5. OpenAPI schema returns 259 routes with 3 news endpoints
6. All news endpoints return HTTP 200 with valid JSON

______________________________________________________________________

**Last Updated**: October 14, 2025, 3:15 PM CDT\

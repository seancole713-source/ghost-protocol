...
**Target Grade:**78% → 90% (3 patches)**Estimated Time:**5 days total
...
### File to Modify**`wolf_app.py`** lines 1450-1460 (GPS threshold constants) + lines 7800-7850 (hunter feed endpoint)
...
### Solution Applied (Session 4)**File:**`static/cockpit_v3.js` lines 354-374
...
### User Action Required**Check browser console (F12) and report logs:**```text
...
### Conditional Fix (After User Feedback)**IF sentiment is string instead of number:**```javascript
...
```text**IF all predictions are "flat" (0.0 sentiment):**- Check backend signal generation thresholds (may be too high)

- Lower BUY/SELL threshold from ±2.0% to ±1.5%**IF sentiment field is missing:**- Fix backend news feed endpoint to always include sentiment
...
---**Patches Created:**December 2, 2025**Author:**GitHub Copilot (Claude Sonnet 4.5)**Status:**✅ READY TO DEPLOY**Full Report:** `GHOST_PREDICTION_ENGINE_AUTOPSY.md`

...
### BTC Prediction Test**Endpoint**: `POST /api/predict/run?symbol=BTC`
...
### AAPL Prediction Test**Endpoint**: `POST /api/predict/run?symbol=AAPL`
...
- Stock provider chain exhausted all options:
  ...
  4.**polygon**- May have rate limits or TSLA-specific issues**Evidence this is NOT a Ghost bug**:
...
**Stock Status**: ⚠️ **PARTIAL OPERATIONAL**- ✅ AAPL working
...
- `DATABASE_URL="${{Postgres.DATABASE_URL}}"` ✅**Write Flow**:
...
- Updated after each prediction creation**History Endpoint**(when called):

- Uses `_PREDICTION_STORE.get_prediction_history()`
- Routes to `PostgresBackend.get_prediction_history()`
- Queries Postgres `predictions` and `prediction_points` tables**No SQLite Read Paths Detected**:
...
**NO**- Current behavior is correct given provider limitations.**Reasons**:
...
### What's Happening with TSLA/MSFT**Provider Chain Exhaustion**
...
4.**polygon**→ Fails (rate limits or data unavailable)**Why AAPL Works but TSLA/MSFT Don't**:
...
**Market Status**: **CLOSED**(US stock market)**Market Hours**: Mon-Fri 9:30 AM - 4:00 PM ET (14:30-21:00 UTC)
...
3.**Extended hours data**- Polygon may include pre-market/after-hours quotes for AAPL**Evidence**: `"current_price": 278.85` suggests valid price retrieval
...
1. **Wait 1 hour and retry TSLA/MSFT**- Provider rate limits typically reset hourly
   - Confirm if issue is temporary or persistent


1.**Test during market hours (Mon-Fri 14:30-21:00 UTC)**- Verify TSLA/MSFT work with real-time data
...
1.**Enable PRICE_FALLBACK_PREVCLOSE=1**```bash
...
1.**Upgrade Polygon API Tier**- Free tier: 5 requests/minute, 15-min delay

   - Starter tier ($29/mo): 100 requests/minute, real-time data
   - Would eliminate most rate limit issues


1.**Add ALPHAVANTAGE_API_KEY**- Free tier: 25 requests/day
...
| 23:09:55 | GET /api/v3/predictions/latest?symbol=AAPL | ✅ Retrieved cached |**Total Duration**: ~1 minute

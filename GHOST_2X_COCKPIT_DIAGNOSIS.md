...
The Ghost Intelligence Cockpit is displaying "DELISTED MODE PROVIDER UNAUTHORIZED" with virtually all panels showing empty data (`—`, `NO_DATA`, or empty arrays). This comprehensive audit identifies:

1. **Root Causes** of the "broken" appearance
2. **Missing Data Wiring** preventing panels from populating
3. **Implementation Plan** to restore full functionality

**Critical Finding**: The system is NOT broken - it's **under-configured and under-wired**. The backend endpoints exist but return empty/null data because:
...
2. **PROVIDER UNAUTHORIZED Badge** (line 177 in cockpit.html):
...
3. **Focus Display** (line 1897 in cockpit.html):
...
2. **Add Error Surfacing** (cockpit.html, multiple locations):
...
**Backend**: `/api/goals` endpoint returning PnL by time period
...
4. Phase 2.2: Wire World Context (SPY/VIX/news)

**High Value** (core functionality):
5. Phase 2.3: Accuracy Ledger (proves predictions work)
6. Phase 2.4: Market Regime (trading intelligence)
7. Phase 3.2: Ghost Score (GPS) panel (brand identity)

**Nice to Have** (complete the baseline):
8. Phase 3: Remaining baseline modules
9. Phase 4: Admin config hardening
...
3. _generate_48h_forecast() immediately fails at line 3009:
   ...
   ```
4. `get_wolf_price()`, `get_price_quorum()`, `get_vip_price()` all return `None`
5. Root cause: Missing API keys → HTTP 401 → price provider failures
...
- **Ghost Score (GPS)**: **NEW** live 0-10 indicator for cockpit
  ...
  - Should be computed every tick (SSE stream)

**Recommendation**: Implement GPS as separate from Ghost Score V2:
```python

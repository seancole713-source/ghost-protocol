...
2. **Telegram Integration** (`core/telegram_alerts.py`, +75 lines)
...
5. **Test Suite** (`tests/test_movers.py`, 206 lines)
...
6. **Documentation** (`docs/MOVERS_README.md`, 520 lines)
...
**Status**: NOT YET RUN (server restart required from previous deployment)

## Known Limitations

1. **Historical Price Calculations**
   - `pct_1h` and `pct_24h` currently hardcoded to 0.0
   - Need OHLCV integration for real percentage calculations
   - `ohlcv_func` parameter exists but passed as None

2. **Volume Baselines**
   - `get_volume_baseline()` can't work without OHLCV data
   - Function signature ready, needs wiring to historical data source

3. **Universe Simplified**
   - Top crypto/stocks use static lists instead of live API calls
   - Future: Integrate CoinGecko top coins and screener top stocks endpoints

4. **SSE Integration**
   - Not added to `/api/cockpit/stream` endpoint
...
2. **Run Validation Plan** (5 steps per MOVERS_README.md)
...
4. **Optional Enhancements (Phase 2)**
   - Wire OHLCV integration for real pct_1h/pct_24h
...
> "VALIDATION PLAN (run these after deploy; log results to ./validation_movers.log)"
> "Never claim success without passing acceptance tests"
...
- Deployment: ⏳ Code committed but server not restarted

**Blocker:**
Server restart required to load new code. Previous deployment (Phase 100% ops) also blocked by same PID 1 constraint.
